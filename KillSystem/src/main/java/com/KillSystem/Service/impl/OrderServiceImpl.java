...
public class OrderServiceImpl implements OrderService{
	...
	@Override
	public boolean orderIsExistByTelnumAndGoodsId(Order order) {
		// TODO Auto-generated method stub
		return orderDao.selectBytelnumAndgoodsid(order.getTel_num(), order.getGoods_id()) == null ? false : true;
	}
    ...
    public ServerResponse aliCallback(Map<String,String> params){
        ...
        if(orderDao.getPayState(order) == null || orderDao.getPayState(order) == "-1") {
        	if(orderDao.getPayState(order) == null){
            	goodsDao.setBackGoodsStock(order);
            	orderDao.setPayStateFailed(order);//防止支付宝5分钟内多次回调,失效时间5分钟
            }
        	return ServerResponse.createBySuccessMessage("该支付订单已失效");
        }
        ...
        if("TRADE_SUCCESS".equals(tradeStatus)){
        	...
        	goodsDao.setBackGoodsStock(order);
        	orderDao.setPayStateFailed(order);
